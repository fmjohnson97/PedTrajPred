...
def train(net, loader, args):
    ...
    for e in range(args.epochs):
        ...
        torch.save(net.state_dict(), 'simpleRegNet_allDiffsClosest_' + str(args.num_people) + 'people_' + str(args.window)+'window.pt')
...
if __name__ == '__main__':
    args=get_args()
    dataset = TSNEGT(args.path + 'allDiffsClosest_' + str(args.num_people) + 'thresh_'+str(args.window)+'window.csv', args.num_clusters)
    ...
    if args.train:
        print('training loop')
        # net.load_state_dict(torch.load('simpleRegNet_allDiffsClosest_' + str(args.num_people) + 'people_' + str(args.window)+'window.pt'))
    ...
    dataset = TSNEGT(args.path + 'allDiffsClosest_' + str(args.num_people) + 'thresh_' + str(args.window) + 'window.csv',
                     args.num_clusters)
    ...
    dataset = TSNEGT(args.path + 'allDiffsClosest_' + str(args.num_people) + 'thresh_'+str(args.window)+'window.csv', args.num_clusters, split='test')
    # dataset = TSNEGT(args.path + 'socData_' + str(args.num_people) + 'thresh_'+ str(args.num_people) +'group_8window.csv', args.num_clusters, split='test')
    loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True)
    test(net,loader,args)
    graphPreds(net, loader, args)
