...
def get_args():
    ...
    parser.add_argument('--traj_thresh', default=2,  help='number of trajectories in each data point')
    parser.add_argument('--social_thresh', default=2,  help='number of social groups in each data point')
...
if __name__=='__main__':
    ...
    if args.traj_thresh is not None:
        ...
        for i,x in enumerate(data['pos']):
            ...
            positions.extend(x.reshape(-1,1))
    ...
    if args.traj_thresh is not None:
        ...
        plt.title(str(args.traj_thresh)+" Traj Data, "+str(args.num_clusters)+" Clusters, Len "+str(args.input_window+args.output_window))
        plt.show()
    ...
    if args.social_thresh is not None:
        ...
        kmeans = KMeans(n_clusters=args.num_clusters, random_state=0).fit(socData)
        centers = kmeans.cluster_centers_

        print('plotting the data')
        plt.scatter(socData[:, 0], socData[:, 1], c=kmeans.labels_, alpha=0.5)
        plt.scatter(centers[:, 0], centers[:, 1], c='r')
        plt.title(str(args.soc_thresh) + " Soc Dist Data, " + str(args.num_clusters) + " Clusters, Len " +
                  str(args.input_window + args.output_window) + ", " + str(args.group_size) + "Group Size, "+
                  str(args.dist_thresh)+" Dist thresh")
        plt.show()
