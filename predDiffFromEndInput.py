...
def train(args, net, device, tsne_nets):
    ...
    for e in tqdm(range(args.epochs)):
        ...
        for name in ['ETH', 'ETH_Hotel', 'UCY_Zara1', 'UCY_Zara2']:
            ...
            for data in loader:
                if data['pos'].nelement()>0:
                    ...
                    # input_scene = scene[:,:args.input_window,:]
                    output, latent = net(input_scene.reshape(-1, (args.input_window) * 4).float().to(device))
                    opt.zero_grad()
                    loss = loss_func(output, torch.diff(scene[:, (args.input_window-1):, :], axis=1).reshape(-1,(args.output_window)*2).float().to(device))
                    loss.backward()
                    opt.step()
                    scheduler.step()
        ...
        if np.mean(avgLoss)<min_loss:
            ...
            torch.save(net.state_dict(),'simpleNet_diffsVel_best.pt')
            min_loss=np.mean(avgLoss)
        torch.save(net.state_dict(), 'simpleNet_diffsVel_latest.pt')
    return net
        ...
        for data in loader:
            if data['pos'].nelement() > 0:
                ...
                with torch.no_grad():
                    ...
                    # input_scene = scene[:, :args.input_window, :]
                    output, latent = net(input_scene.reshape(-1, (args.input_window) * 4).float().to(device))
                    output = output.reshape(-1, args.output_window, 2)
                    output = torch.cumsum(output, dim=1) + input_scene[:,-1:,:2]
                    loss = loss_func(output, scene[:,args.input_window:,:]).float().to(device)
                avgLoss.append(loss.item())
...
if __name__=='__main__':
    ...
    net.load_state_dict(torch.load('simpleNet_diffsVel_best.pt'))
    net.eval()
    net = net.to(device)
    for i in N:
        temp = SimpleRegNetwork(i * (args.input_window - 1) * 2).eval()
